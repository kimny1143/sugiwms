...
if uploaded_file is not None:
    ...
    # 時間帯ごとの各作業数の平均を計算
    df_avg = df.groupby('hour')[['作業数', '作業数2', '作業数3']].mean().reset_index().melt('hour', var_name='category', value_name='average')

    # Altairを使用してグラフをプロット
    chart = alt.Chart(df_avg).mark_line().encode(
        x='hour',
        y=alt.Y('average', axis=alt.Axis(title='平均作業数')),
        color=alt.Color('category', legend=alt.Legend(title="作業種類")),
        tooltip=['hour', 'average', 'category']
    ).interactive()
