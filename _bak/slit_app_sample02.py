...
if uploaded_file is not None:
    ...
    df['終了datetime'] = pd.to_datetime(df['日付'] + ' ' + df['終了時刻'])
    ...
    for work_number in ['作業数', '作業数2', '作業数3']:
        ...
        fig, ax = plt.subplots()
        sns.lineplot(x=efficiency_by_hour.index, y=efficiency_by_hour.values, ax=ax)
        ax.set(title=f'Hourly {work_number}', xlabel='Hour of the Day', ylabel=f'Average {work_number}')
        st.pyplot(fig)
else:
